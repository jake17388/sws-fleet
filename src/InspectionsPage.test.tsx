...
describe('Inspections workspace', () => {
  it('shows the seeded pre-trip template and template administration', async () => {
    ...
    expect(screen.getByRole('button', { name: /Duplicate SWS Truck Pre-Trip Inspection/ })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Edit SWS Truck Pre-Trip Inspection/ })).toBeInTheDocument()
  })
  ...
  it('starts, saves, resumes, validates, and submits an inspection', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'SWS Truck Pre-Trip Inspection' })).toBeInTheDocument()
