...
import { fromDatabaseVehicle, toDatabaseVehicle, isDatabaseId, loadVehicles, saveVehicles, type Vehicle } from './vehicleModel'
...
export async function persistVehicle(vehicle: Vehicle): Promise<Vehicle> {
  if (!supabase) {
    ...
    saveVehicles(current.some(v => v.id === saved.id) ? current.map(v => v.id === saved.id ? saved : v) : [...current, saved])
    return saved
  }
