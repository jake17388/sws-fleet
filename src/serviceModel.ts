...
export function validateService(record: ServiceRecord): string {
  ...
  if (record.dueMeter !== null && (!Number.isFinite(record.dueMeter) || record.dueMeter < 0))
    return "Enter a valid due meter.";
  if (record.completedDate) {
    if (!validDate(record.completedDate) || record.completedDate > today())
      return "Enter a completion date on or before today.";
    if (
      record.completedMeter === null ||
      !Number.isFinite(record.completedMeter) ||
      record.completedMeter < 0
    )
      return "Enter a valid completion meter.";
    if (record.cost === null || !Number.isFinite(record.cost) || record.cost < 0)
      return "Enter a valid cost, including zero for no charge.";
  }
  return "";
}
export function serviceStatus(record: ServiceRecord, currentMeter: number, date = today()) {
  ...
  if (
    ...
    (record.dueMeter !== null && currentMeter > record.dueMeter)
  )
    return "Overdue";
  if (record.dueMeter !== null && currentMeter === record.dueMeter) return "Due now";
  if (record.dueDate === date) return "Due today";
  if (record.dueDate && (Date.parse(record.dueDate) - Date.parse(date)) / 86400000 <= 7)
    return "Due soon";
  return "Scheduled";
}
...
const fromRow = (row: Record<string, unknown>): ServiceRecord => ({
  ...
  dueMeter: row.due_meter == null ? null : Number(row.due_meter),
  meterUnit: row.meter_unit as "mi" | "hr",
  notes: String(row.notes ?? ""),
  completedDate: row.completed_date == null ? null : String(row.completed_date),
  completedMeter: row.completed_meter == null ? null : Number(row.completed_meter),
...
export async function persistService(record: ServiceRecord): Promise<ServiceRecord> {
  ...
  if (!supabase) {
    ...
    localStorage.setItem(
      key,
      JSON.stringify(
        record.id ? records.map((r) => (r.id === record.id ? saved : r)) : [saved, ...records],
      ),
    );
    return saved;
  }
  const row = {
    ...
    due_meter: record.dueMeter,
    meter_unit: record.meterUnit,
    notes: record.notes,
    completed_date: record.completedDate,
    completed_meter: record.completedMeter,
    cost: record.cost,
    provider: record.provider,
  };
  const request = record.id
    ? supabase.from("service_records").update(row).eq("id", record.id).is("completed_date", null)
    : supabase.from("service_records").insert(row);
