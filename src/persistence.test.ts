...
const { from, single, insert, update, eq } = vi.hoisted(() => {
  ...
  return {
    single,
    eq,
    insert: vi.fn((_row: unknown) => ({ select })),
    update: vi.fn(() => ({ eq })),
    from: vi.fn(),
  };
});
...
it("inserts only the new vehicle and returns its database id", async () => {
  single.mockResolvedValue({
    data: { id: "8b755398-9c9b-4d93-8bba-e55674a99f45", name: "New truck" },
    error: null,
  });
...
it("propagates rejected saves instead of claiming success", async () => {
  ...
  await expect(persistVehicle({ ...sampleVehicles[0], id: "new" })).rejects.toThrow(
    "Permission denied",
  );
});
