...
it("classifies date and meter reminders including due today", () => {
  ...
  expect(serviceStatus({ ...service, dueDate: "", dueMeter: 100 }, 100, "2026-09-14")).toBe(
    "Due now",
  );
  expect(serviceStatus({ ...service, dueDate: "", dueMeter: 200 }, 100, "2026-09-14")).toBe(
    "Scheduled",
  );
  expect(serviceStatus({ ...service, completedDate: "2026-09-14" }, 100, "2026-09-21")).toBe(
    "Completed",
  );
});
