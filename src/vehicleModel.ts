...
export const validateVehicle = (input: Partial<VehicleInput>) => {
  ...
  if (input.vin && !/^[A-HJ-NPR-Z0-9]{11,17}$/i.test(input.vin))
    errors.vin = "VIN must be 11–17 valid characters";
  if (!vehicleStatuses.includes(input.status as VehicleStatus))
    errors.status = "Choose a valid status";
  if (!vehicleTypes.includes(input.type as VehicleType)) errors.type = "Choose a valid type";
  if (!Number.isFinite(input.currentMeter) || input.currentMeter! < 0)
    errors.currentMeter = "Meter must be zero or greater";
  return errors;
};
...
export const sampleVehicles: Vehicle[] = [
  [
    ...
    "mi",
    "CJ43300",
    "/sws-fleet/vehicles/double-2015.jpg",
  ],
  [
    "2016 Flatbed",
    2016,
    ...
    "mi",
    "CK47519",
    "/sws-fleet/vehicles/flatbed-2016.jpg",
  ],
  [
    "2018 Altec Crane",
    2018,
    ...
    "mi",
    "CMO6222",
    "/sws-fleet/vehicles/crane-2018.jpg",
  ],
  [
    ...
    "mi",
    "HMA 02D",
    "/sws-fleet/vehicles/trailer-dump.jpg",
  ],
].map(
  (
    [
...
export const saveVehicles = (vehicles: Vehicle[]) => {
  if (typeof localStorage !== "undefined") localStorage.setItem(KEY, JSON.stringify(vehicles));
};
...
export const fromDatabaseVehicle = (row: Record<string, unknown>): Vehicle => ({
  ...
  group: String(row.vehicle_group ?? ""),
  currentMeter: Number(row.current_meter ?? 0),
...
export const toDatabaseVehicle = (vehicle: Vehicle) => ({
  ...
  type: vehicle.type,
  vehicle_group: vehicle.group || null,
  current_meter: vehicle.currentMeter,
  meter_unit: vehicle.meterUnit,
  license_plate: vehicle.licensePlate || null,
  photo_url: vehicle.photoUrl || null,
});

export const isDatabaseId = (id: string) =>
