...
const template: InspectionTemplate = {
  ...
  description: "Daily truck safety check",
  vehicleTypes: ["Truck"],
  vehicleIds: [],
  cadenceDays: 1,
  archivedAt: null,
  ...
  items: [
    {
      ...
      required: true,
      instructions: "Confirm firm pedal pressure.",
      position: 0,
    },
    {
      id: "item-2",
...
describe("inspection templates", () => {
  ...
  it("validates template names, assignments, and items", () => {
    ...
    expect(validateInspectionTemplate({ ...template, vehicleTypes: [], vehicleIds: [] })).toMatch(
      /assign/i,
    );
...
describe("inspection completion", () => {
  it("lists unanswered required items", () => {
    expect(requiredInspectionItemsMissing(template, inspection)).toEqual([
      "Service brakes",
      "Odometer",
    ]);
  });
  ...
  it("derives passed and failed submission states", () => {
    ...
    expect(
      inspectionStatus(template, {
        ...complete,
        responses: [
          { itemId: "item-1", value: "fail", notes: "", photoPaths: [] },
  ...
  it("detects overdue recurring inspections", () => {
    expect(isInspectionOverdue(template, [], "vehicle-1", new Date("2026-09-16T12:00:00Z"))).toBe(
      true,
    );
    expect(
      isInspectionOverdue(
        template,
        [{ ...inspection, status: "Passed", submittedAt: "2026-09-16T08:00:00Z" }],
        "vehicle-1",
