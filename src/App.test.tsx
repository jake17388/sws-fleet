...
describe("SWS Fleet shell", () => {
  ...
  it("uses the supplied vehicle and service icons with spaced sidebar labels", () => {
    ...
    expect(vehicleLink).toHaveStyle({
      display: "grid",
  ...
  it("shows the dashboard overview", () => {
    render(<App />);
    expect(screen.getByText("Good morning, Jake")).toBeInTheDocument();
  ...
  it("switches workspace from navigation", () => {
    render(<App />);
    fireEvent.click(screen.getAllByRole("link", { name: "Vehicles" })[0]);
    expect(screen.getAllByRole("heading", { name: "Vehicles" }).length).toBeGreaterThan(0);
  });
  ...
  it("shows vehicle fields and lets an administrator watch a vehicle", () => {
    ...
    expect(
      screen.getAllByRole("button", { name: "Unwatch 2015 Double Bucket" }).length,
    ).toBeGreaterThan(0);
  });
