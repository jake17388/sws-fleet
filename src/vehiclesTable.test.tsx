...
beforeEach(() => {
  ...
  saveVehicles(
    Array.from({ length: 12 }, (_, index) => ({
      ...sampleVehicles[0],
      id: `truck-${index}`,
      name: `Truck ${String(index + 1).padStart(2, "0")}`,
...
it("shows the entire fleet in one table and filters vehicles beyond the former page limit", async () => {
  ...
  fireEvent.change(screen.getByLabelText("Filter by status"), {
    target: { value: "In maintenance" },
  });
  expect(within(screen.getByRole("table")).getAllByRole("row")).toHaveLength(2);
  fireEvent.click(screen.getByRole("button", { name: "Open Truck 12" }));
  expect(screen.getByRole("link", { name: "Open service records" })).toHaveAttribute(
    "href",
    "/service?vehicle=truck-11",
  );
});
